...
assert.match(scrims, /api\/community\/ranking-titles\?players=/);
assert.doesNotMatch(scrims, /api\/community\/players\/.*limit=1/);
assert.doesNotMatch(scrims, /row\.champion|player\.champion/);
assert.match(scrims, /aria-label="칭호 없음">◇/);
