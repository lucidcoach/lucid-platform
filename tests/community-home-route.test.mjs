...
assert.match(page, /내 전적과 진행 중인 내전을 <span>한눈에<\/span> 확인하세요/);
...
assert.doesNotMatch(homeCss, /home-live-player \.current-form\{display:none/);
assert.match(recruitment, /apiGet\("\/api\/community\/recruitments"\)/);
assert.match(page, /id="recruitmentPreviewBtn"[\s\S]*?hidden>모집 큐 예시<\/button>/);
assert.match(recruitment, /isCommunityAdmin\(\)[\s\S]*button\.hidden = !allowed/);
assert.match(recruitment, /showingExample \? \[exampleQueue, \.\.\.queues\] : queues/);
...
assert.match(entry, /initialScrimsRoute[\s\S]*switchView\("recent"\)/);
assert.match(scrims, /fetch\("\.\.\/index\.html"\)/);
assert.match(scrims, /import\("\.\.\/js\/app-report6\.js\?v=20260915queuepreview1"\)/);
assert.doesNotMatch(scrims, /location\.replace|searchParams\.set/);
