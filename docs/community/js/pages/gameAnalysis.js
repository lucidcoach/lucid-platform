...
import { canAnalyzePlayer, getAnalysisIdentity, isCommunityAdmin } from "../auth.js?v=20260905ae";
...
export async function renderCompactMatchAnalysis(detail={},target){
  ...
  if(!canAnalyzePlayer(userId,guildId)){
    target.innerHTML=`<div class="compact-analysis-empty"><strong>분석 권한이 필요합니다.</strong><span>${isCommunityAdmin()?"":"로그인 후 내 ID를 등록하면 내 경기만 분석할 수 있습니다."}</span></div>`; return;
  }
...
async function renderAnalysis({userId="",guildId="",matchId="",champion="",role=""}={}){
  ...
  if(userId && !canAnalyzePlayer(userId,guildId)){ target.innerHTML=`<div class="analysis-empty-inline"><strong>분석 권한이 필요합니다.</strong><span>일반 계정은 로그인 후 등록한 본인 내전 기록만 분석할 수 있습니다. 관리자는 모든 유저를 분석할 수 있습니다.</span></div>`; return; }
