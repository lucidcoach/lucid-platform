...
function renderRetroRoflActions(actions=[]){
  ...
  target.querySelectorAll(".retroactive-card").forEach((card,index)=>{const row=actions[index],r=row?.result||{},p=row?.payload||{};if(row?.action!=="inspect_retroactive_rofl"||row.status!=="completed"||!(r.aram||String(r.matchId||"").includes("-aram-")))return;card.querySelector("[data-retro-apply]")?.remove();const footer=card.querySelector("footer"),base=`data-retro-apply="${esc(r.sha256||p.sha256)}" data-retro-file="${esc(r.filename||p.filename)}" data-retro-class="${esc(r.classification)}"`;if(r.matchId)footer.insertAdjacentHTML("beforeend",`<button type="button" ${base} data-retro-mode="replace" data-retro-target="${esc(r.matchId)}">기존 칼바람 기록 바로잡기</button>`);footer.insertAdjacentHTML("beforeend",`<button type="button" ${base} data-retro-mode="new">별도 칼바람 경기 추가 (MMR 없음)</button>`);});
