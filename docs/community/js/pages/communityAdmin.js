...
let titleRows=[],levelIconState=null,editingTitleKey="",titlePreviewUrl="",titlePreviewValid=false;
const TITLE_ICON_TYPES=["image/png","image/jpeg","image/webp"],TITLE_ICON_MAX_BYTES=512*1024,TITLE_ICON_SIZE=256;
function titlePanel(){return shell(`${panelTitle("칭호 / 업적","봇이 실제 사용하는 칭호 정의와 달성 조건입니다.")}<section class="admin-work-panel admin-level-icon-panel"><div><small>내전 레벨 아이콘</small><h3>프로필 공용 아이콘</h3><p>내전 레벨 배지에만 사용 · PNG/JPG/WebP 정사각형 256 × 256 · 512KB 이하</p></div><form id="levelIconForm"><span id="levelIconPreview" class="admin-level-icon-preview">🎮</span><input name="icon" type="file" accept="image/png,image/jpeg,image/webp" required><button class="admin-primary" type="submit">아이콘 변경</button></form><small id="levelIconStatus">이미지를 올리기 전에는 🎮 이모지를 사용합니다.</small></section><section class="admin-work-panel"><div class="admin-title-tools"><input id="titleSearch" type="search" placeholder="칭호 또는 조건 검색"><select id="titleCategory"><option value="">전체 대분류</option></select><select id="titleSubcategory"><option value="">전체 세부분류</option></select><b id="titleCount">0개</b></div><div id="titleCatalog" class="admin-title-grid"><div class="admin-empty-admin"><strong>불러오는 중...</strong></div></div></section><dialog id="titleIconDialog" class="admin-title-icon-dialog"><form id="titleIconForm"><button class="admin-dialog-close" type="button" data-title-icon-close aria-label="닫기">×</button><small>칭호 전용 이미지</small><h3 id="titleIconName"></h3><b id="titleIconState" class="admin-title-icon-state"></b><span id="titleIconPreviewLabel">현재 이미지</span><div id="titleIconPreview" class="admin-title-icon-preview"></div><input name="icon" type="file" accept="image/png,image/jpeg,image/webp"><small id="titleIconFileInfo">PNG/JPG/WebP · 256 × 256 · 512KB 이하</small><small id="titleIconStatus" aria-live="polite"></small><div class="admin-title-icon-sample"><span id="titleIconSample"></span><strong id="titleIconSampleName"></strong></div><div class="admin-title-icon-actions"><button class="admin-primary" type="submit" data-title-icon-save disabled>저장</button><button type="button" data-title-icon-cancel disabled>선택 취소</button><button type="button" data-title-icon-remove>기본 이미지로 복원</button></div></form></dialog>`);}
function titleIconMarkup(row){if(row.customIconUrl)return `<img src="${esc(apiUrl(row.customIconUrl))}" alt="${esc(row.name)} 아이콘">`;const champions=row.championNames||[];if(row.iconSource==="champion"&&champions.length)return champions.map(name=>{const url=championIcon(name);return url?`<img src="${esc(url)}" alt="${esc(name)}">`:"";}).join("");return row.iconSource==="emoji"?`<span>${esc(row.iconEmoji)}</span>`:"";}
function renderTitleCatalog(){const search=String(document.getElementById("titleSearch")?.value||"").toLowerCase(),category=document.getElementById("titleCategory")?.value||"",subcategory=document.getElementById("titleSubcategory")?.value||"",rows=titleRows.filter(row=>(!category||(row.primaryCategory||row.category)===category)&&(!subcategory||row.subcategory===subcategory)&&(!search||`${row.name} ${row.condition} ${row.champion||""} ${row.defaultTitle||""} ${row.overrideTitle||""} ${row.primaryCategory||""} ${row.subcategory||""} ${(row.holders||[]).map(holder=>holder.name).join(" ")}`.toLowerCase().includes(search)));const count=document.getElementById("titleCount"),target=document.getElementById("titleCatalog");if(count)count.textContent=`${rows.length}개`;if(!target)return;target.innerHTML=rows.map(row=>{const mastery=row.subcategory==="챔피언 숙련"||row.category==="챔피언 숙련",kind={normal:"일반",first:"서버 최초",multi:"복합 서버 최초"}[row.masteryKind]||row.masteryKind||"일반",source={champion:"챔피언 아이콘",custom:"커스텀",emoji:"기본"}[row.customIcon?"custom":row.iconSource]||"기본",primary=row.primaryCategory||row.category||"기타",holderNames=(row.holders||[]).map(holder=>holder.name).filter(Boolean),holderText=holderNames.length?holderNames.join(", "):(row.claimed?(row.claimantName||row.claimantId||"획득 완료"):"미획득");return `<article><header><div class="admin-title-icon">${titleIconMarkup(row)}</div><div class="admin-title-heading"><small><span>${esc(primary)}</span>${row.subcategory?`<span>${esc(row.subcategory)}</span>`:""}${row.isFirstAchievement?`<span>최초 달성</span>`:""}</small><strong>${esc(row.name)}</strong><span class="admin-title-source">${esc(source)}</span></div><button class="admin-title-icon-edit" type="button" data-title-icon-edit="${esc(row.id)}">이미지 편집</button></header><div class="admin-title-body"><div><small>조건:</small><span>${esc(row.condition||"조건 정보 없음")}</span></div>${row.isFirstAchievement?`<div class="admin-title-holders"><small>보유자:</small><span>${esc(holderText)}</span></div>`:""}${mastery?`<div><small>챔피언:</small><span>${esc(row.champion||"모든 챔피언")} · ${Number(row.games||0)}판 · ${esc(kind)}</span></div><div><small>기본 칭호:</small><span>${esc(row.defaultTitle||row.name)}</span></div>${row.overrideTitle?`<div><small>특수 칭호:</small><span>${esc(row.overrideTitle)}</span></div>`:""}${row.claimKey?`<div><small>최초 획득:</small><span>${esc(row.season||"현재 시즌")} · ${row.claimed?esc(row.claimantName||row.claimantId||"획득 완료"):"미획득"}</span></div>`:""}`:""}</div></article>`;}).join("")||`<div class="admin-empty-admin"><strong>조건에 맞는 칭호가 없습니다.</strong></div>`;target.querySelectorAll("[data-title-icon-edit]").forEach(button=>button.addEventListener("click",()=>openTitleIconEditor(button.dataset.titleIconEdit)));}
function clearTitlePreviewUrl(){if(titlePreviewUrl){URL.revokeObjectURL(titlePreviewUrl);titlePreviewUrl="";}}
function closeTitleIconEditor(){clearTitlePreviewUrl();titlePreviewValid=false;document.getElementById("titleIconDialog")?.close();}
function titleIconFileInfo(file,width,height){const type={"image/png":"PNG","image/jpeg":"JPG","image/webp":"WebP"}[file.type]||file.type||"알 수 없는 형식";return `${file.name} · ${width||"?"} × ${height||"?"} · ${Math.ceil(file.size/1024)}KB · ${type}`;}
async function validateTitleIcon(file,url){const errors=[];let width=0,height=0;try{[width,height]=await new Promise((resolve,reject)=>{const image=new Image();image.onload=()=>resolve([image.naturalWidth,image.naturalHeight]);image.onerror=()=>reject(new Error("이미지 파일을 읽을 수 없습니다."));image.src=url;});}catch(error){errors.push(error.message);}if(width&&height&&(width!==TITLE_ICON_SIZE||height!==TITLE_ICON_SIZE))errors.push("이미지 크기는 256×256이어야 합니다.");if(file.size>TITLE_ICON_MAX_BYTES)errors.push("이미지는 512KB 이하여야 합니다.");return {width,height,errors,valid:!errors.length};}
function showTitleIconPreview(row,markup,label,details=""){document.getElementById("titleIconPreview").innerHTML=markup||"등록된 이미지 없음";document.getElementById("titleIconPreviewLabel").textContent=label;document.getElementById("titleIconFileInfo").textContent=details||"PNG/JPG/WebP · 256 × 256 · 512KB 이하";document.getElementById("titleIconSample").innerHTML=markup||titleIconMarkup(row)||"🏷️";document.getElementById("titleIconSampleName").textContent=row.name;}
function resetTitleIconSelection(){const row=titleRows.find(item=>item.id===editingTitleKey),dialog=document.getElementById("titleIconDialog");if(!row||!dialog)return;clearTitlePreviewUrl();titlePreviewValid=false;dialog.querySelector("input[name=icon]").value="";dialog.querySelector("[data-title-icon-save]").disabled=true;dialog.querySelector("[data-title-icon-cancel]").disabled=true;const status=document.getElementById("titleIconStatus");status.textContent="";status.classList.remove("is-error");showTitleIconPreview(row,titleIconMarkup(row),row.customIcon?"현재 커스텀 이미지":"기본 이미지");}
function openTitleIconEditor(key){const row=titleRows.find(item=>item.id===key),dialog=document.getElementById("titleIconDialog");if(!row||!dialog)return;clearTitlePreviewUrl();titlePreviewValid=false;editingTitleKey=key;dialog.querySelector("form")?.reset();document.getElementById("titleIconName").textContent=row.name;document.getElementById("titleIconState").textContent=row.customIcon?"커스텀 이미지 사용 중":"기본 이미지 사용 중";dialog.querySelector("[data-title-icon-save]").disabled=true;dialog.querySelector("[data-title-icon-cancel]").disabled=true;dialog.querySelector("[data-title-icon-remove]").disabled=!row.customIcon;resetTitleIconSelection();dialog.showModal();}
async function previewTitleIcon(file){const row=titleRows.find(item=>item.id===editingTitleKey),dialog=document.getElementById("titleIconDialog"),status=document.getElementById("titleIconStatus");if(!row||!dialog)return;if(!file?.size)return resetTitleIconSelection();clearTitlePreviewUrl();titlePreviewValid=false;dialog.querySelector("[data-title-icon-save]").disabled=true;dialog.querySelector("[data-title-icon-cancel]").disabled=false;if(!TITLE_ICON_TYPES.includes(file.type)){showTitleIconPreview(row,titleIconMarkup(row),row.customIcon?"현재 이미지":"기본 이미지",titleIconFileInfo(file,0,0));status.textContent="❌ 지원하지 않는 이미지 형식입니다. PNG, JPG 또는 WebP를 선택해주세요.";status.classList.add("is-error");return;}const url=URL.createObjectURL(file);titlePreviewUrl=url;const markup=`<img src="${esc(url)}" alt="변경 미리보기">`;showTitleIconPreview(row,markup,"변경 미리보기",titleIconFileInfo(file,0,0));const preview=await validateTitleIcon(file,url);if(titlePreviewUrl!==url)return;titlePreviewValid=preview.valid;showTitleIconPreview(row,markup,"변경 미리보기",titleIconFileInfo(file,preview.width,preview.height));status.textContent=preview.valid?"저장 가능한 이미지입니다.":preview.errors.map(message=>`❌ ${message}`).join(" ");status.classList.toggle("is-error",!preview.valid);dialog.querySelector("[data-title-icon-save]").disabled=!preview.valid;}
async function saveTitleIcon(event){event.preventDefault();const status=document.getElementById("titleIconStatus"),file=new FormData(event.currentTarget).get("icon");if(!file?.size||!titlePreviewValid){status.textContent="저장 가능한 이미지를 선택해주세요.";status.classList.add("is-error");return;}try{status.classList.remove("is-error");status.textContent="저장 중...";const body=new FormData();body.append("icon",file);await adminRequest(`/api/community/admin/guilds/${encodeURIComponent(selectedGuild)}/titles/${encodeURIComponent(editingTitleKey)}/icon`,{method:"POST",body});closeTitleIconEditor();await loadTitleCatalog();}catch(error){status.textContent=error.message;status.classList.add("is-error");}}
async function removeTitleIcon(){if(!confirm("현재 칭호 전용 이미지를 제거하고 기본 이미지로 복원할까요?"))return;const status=document.getElementById("titleIconStatus");status.classList.remove("is-error");status.textContent="기본 이미지로 복원 중...";try{await adminRequest(`/api/community/admin/guilds/${encodeURIComponent(selectedGuild)}/titles/${encodeURIComponent(editingTitleKey)}/icon`,{method:"DELETE"});await loadTitleCatalog();const row=titleRows.find(item=>item.id===editingTitleKey),dialog=document.getElementById("titleIconDialog");if(row&&dialog){dialog.querySelector("form")?.reset();document.getElementById("titleIconState").textContent="기본 이미지 사용 중";dialog.querySelector("[data-title-icon-remove]").disabled=true;resetTitleIconSelection();status.textContent="기본 이미지로 복원했습니다.";}}catch(error){status.textContent=error.message;status.classList.add("is-error");}}
...
function updateTitleSubcategories(){const category=document.getElementById("titleCategory")?.value||"",select=document.getElementById("titleSubcategory"),rows=titleRows.filter(row=>!category||(row.primaryCategory||row.category)===category),counts=new Map();rows.forEach(row=>{if(row.subcategory)counts.set(row.subcategory,(counts.get(row.subcategory)||0)+1);});if(select)select.innerHTML=`<option value="">전체 세부분류</option>${[...counts].map(([value,count])=>`<option value="${esc(value)}">${esc(value)} (${count})</option>`).join("")}`;}
async function loadTitleCatalog(){if(!selectedGuild)return;try{const data=await adminRequest(`/api/community/admin/guilds/${encodeURIComponent(selectedGuild)}/titles`);titleRows=data.titles||[];levelIconState=data.levelIcon||null;renderLevelIcon();const select=document.getElementById("titleCategory"),counts=new Map();titleRows.forEach(row=>{const value=row.primaryCategory||row.category;if(value)counts.set(value,(counts.get(value)||0)+1);});if(select)select.innerHTML=`<option value="">전체 대분류</option>${[...counts].map(([value,count])=>`<option value="${esc(value)}">${esc(value)} (${count})</option>`).join("")}`;updateTitleSubcategories();renderTitleCatalog();}catch(error){const target=document.getElementById("titleCatalog");if(target)target.innerHTML=`<div class="admin-empty-admin"><strong>${esc(error.message)}</strong></div>`;}}
...
function renderRetroRoflActions(actions=[]){
  ...
  target.querySelectorAll(".retroactive-card").forEach((card,index)=>{const row=actions[index],r=row?.result||{},p=row?.payload||{};if(row?.action!=="inspect_retroactive_rofl"||row.status!=="completed")return;const footer=card.querySelector("footer");if(r.classification==="D")footer.insertAdjacentHTML("beforeend",`<button type="button" data-retro-reinspect="${esc(r.sha256||p.sha256)}">다시 검증</button>`);if(!(r.aram||String(r.matchId||"").includes("-aram-")))return;card.querySelector("[data-retro-apply]")?.remove();const base=`data-retro-apply="${esc(r.sha256||p.sha256)}" data-retro-file="${esc(r.filename||p.filename)}" data-retro-class="${esc(r.classification)}"`;if(r.matchId)footer.insertAdjacentHTML("beforeend",`<button type="button" ${base} data-retro-mode="replace" data-retro-target="${esc(r.matchId)}">기존 칼바람 기록 바로잡기</button>`);footer.insertAdjacentHTML("beforeend",`<button type="button" ${base} data-retro-mode="new">별도 칼바람 경기 추가 (MMR 없음)</button>`);});
  target.querySelectorAll("[data-retro-reinspect]").forEach(button=>button.addEventListener("click",async()=>{button.disabled=true;try{await adminRequest("/api/community/admin/retroactive-rofl/reinspect",{method:"POST",body:{guildId:selectedGuild,sha256:button.dataset.retroReinspect}});await loadRetroRoflActions();}catch(error){document.getElementById("retroRoflStatus").textContent=error.message;button.disabled=false;}}));
...
async function applyRetroRofl(button){const mode=button.dataset.retroMode||"",completeMissing=button.dataset.retroClass==="A"||mode==="new",message=mode==="new"?"서버 참가자 10명으로 별도 칼바람 전적을 생성합니다. MMR은 소급하지 않습니다. 계속할까요?":mode==="replace"?`기존 경기 ${button.dataset.retroTarget}의 잘못 연결된 상세 전적을 이 ROFL로 교체합니다. 승패·MMR은 유지합니다. 계속할까요?`:completeMissing?"완전 누락 경기입니다. 신규 전적으로 복구하면 승패/MMR이 1회 반영됩니다. 계속할까요?":"기존 승패/MMR은 유지하고 누락된 ROFL 데이터만 복구할까요?";if(!confirm(message))return;button.disabled=true;const status=document.getElementById("retroRoflStatus");try{await adminRequest("/api/community/admin/retroactive-rofl/apply",{method:"POST",body:{guildId:selectedGuild,sha256:button.dataset.retroApply,filename:button.dataset.retroFile,confirmNewMatch:completeMissing,forceNew:mode==="new",targetMatchId:mode==="replace"?button.dataset.retroTarget:"",requestKey:crypto.randomUUID()}});if(status)status.textContent="복구 작업을 요청했습니다.";setTimeout(loadRetroRoflActions,1500);}catch(error){if(status)status.textContent=error.message;button.disabled=false;}}
...
async function loadInquiries(status=""){
  ...
  try{
    ...
    target.querySelectorAll("[data-inquiry-form]").forEach(form=>form.addEventListener("submit",async event=>{event.preventDefault();const values=new FormData(form);const button=form.querySelector("button[type=submit]");button.disabled=true;try{await adminRequest(`/api/community/admin/inquiries/${encodeURIComponent(form.dataset.inquiryForm)}`,{method:"PATCH",body:{status:values.get("status"),assignedTo:values.get("assignedTo"),dueAt:values.get("dueAt"),note:values.get("note"),reply:values.get("reply")}});loadInquiries(status);}catch(error){alert(error.message);button.disabled=false;}}));
...
function renderSection(){
  ...
  if(activeSection==="titles"){loadTitleCatalog();root.querySelector("#titleSearch")?.addEventListener("input",renderTitleCatalog);root.querySelector("#titleCategory")?.addEventListener("change",()=>{updateTitleSubcategories();renderTitleCatalog();});root.querySelector("#titleSubcategory")?.addEventListener("change",renderTitleCatalog);root.querySelector("#titleIconForm")?.addEventListener("submit",saveTitleIcon);root.querySelector("#titleIconForm input[name=icon]")?.addEventListener("change",event=>previewTitleIcon(event.target.files?.[0]));root.querySelector("[data-title-icon-cancel]")?.addEventListener("click",resetTitleIconSelection);root.querySelector("[data-title-icon-remove]")?.addEventListener("click",removeTitleIcon);root.querySelector("[data-title-icon-close]")?.addEventListener("click",closeTitleIconEditor);root.querySelector("#titleIconDialog")?.addEventListener("close",clearTitlePreviewUrl);root.querySelector("#levelIconForm")?.addEventListener("submit",async event=>{event.preventDefault();const status=root.querySelector("#levelIconStatus");try{if(status)status.textContent="업로드 중...";const data=await adminRequest(`/api/community/admin/guilds/${encodeURIComponent(selectedGuild)}/level-icon`,{method:"POST",body:new FormData(event.currentTarget)});levelIconState=data.levelIcon;renderLevelIcon();event.currentTarget.reset();}catch(error){if(status)status.textContent=error.message;}});}
