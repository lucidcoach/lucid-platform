...
async function validateTitleIcon(file){const url=URL.createObjectURL(file),errors=[];let width=0,height=0;try{[width,height]=await new Promise((resolve,reject)=>{const image=new Image();image.onload=()=>resolve([image.naturalWidth,image.naturalHeight]);image.onerror=()=>reject(new Error("이미지 파일을 읽을 수 없습니다."));image.src=url;});}catch(error){errors.push(error.message);}if(!TITLE_ICON_TYPES.includes(file?.type))errors.unshift("지원하지 않는 이미지 형식입니다. PNG, JPG 또는 WebP를 선택해주세요.");if(width&&height&&(width!==TITLE_ICON_SIZE||height!==TITLE_ICON_SIZE))errors.push("이미지는 256×256이어야 합니다.");if(file.size>TITLE_ICON_MAX_BYTES)errors.push("이미지는 512KB 이하여야 합니다.");return {url,width,height,errors,valid:!errors.length};}
...
async function previewTitleIcon(file){const row=titleRows.find(item=>item.id===editingTitleKey),dialog=document.getElementById("titleIconDialog"),status=document.getElementById("titleIconStatus");if(!row||!dialog)return;if(!file?.size)return resetTitleIconSelection();clearTitlePreviewUrl();titlePreviewValid=false;dialog.querySelector("[data-title-icon-save]").disabled=true;const preview=await validateTitleIcon(file);titlePreviewUrl=preview.url;titlePreviewValid=preview.valid;showTitleIconPreview(row,`<img src="${esc(preview.url)}" alt="변경 미리보기">`,"선택 이미지 미리보기",titleIconFileInfo(file,preview.width,preview.height));status.textContent=preview.valid?"저장 가능한 이미지입니다.":preview.errors.map(message=>`❌ ${message}`).join(" ");status.classList.toggle("is-error",!preview.valid);dialog.querySelector("[data-title-icon-save]").disabled=!preview.valid;dialog.querySelector("[data-title-icon-cancel]").disabled=false;}
