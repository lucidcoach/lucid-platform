...
function renderTitleCatalog(){const search=String(document.getElementById("titleSearch")?.value||"").toLowerCase(),category=document.getElementById("titleCategory")?.value||"",rows=titleRows.filter(row=>(!category||row.category===category)&&(!search||`${row.name} ${row.condition} ${row.champion||""} ${row.defaultTitle||""} ${row.overrideTitle||""}`.toLowerCase().includes(search)));const count=document.getElementById("titleCount"),target=document.getElementById("titleCatalog");if(count)count.textContent=`${rows.length}개`;if(!target)return;target.innerHTML=rows.map(row=>{const mastery=row.category==="챔피언 숙련"?`<p>챔피언: ${esc(row.champion||"모든 챔피언")} · 기준: ${Number(row.games||0)}판 · 유형: ${{normal:"일반",first:"서버 최초",multi:"복합 서버 최초"}[row.masteryKind]||esc(row.masteryKind||"")}</p><p>기본 칭호: ${esc(row.defaultTitle||row.name)}${row.overrideTitle?`<br>특수 칭호: ${esc(row.overrideTitle)}`:""}</p>${row.claimKey?`<p>${esc(row.season||"현재 시즌")} 최초 획득: ${row.claimed?esc(row.claimantName||row.claimantId||"획득 완료"):"미획득"}</p>`:""}`:"";return `<article><div class="admin-title-icon">${titleIconMarkup(row)}</div><div><small>${esc(row.category||"기타")}${row.seriesGroup?` · ${esc(row.seriesGroup)}`:""} · ${esc(row.iconSource||"none")}</small><strong>${esc(row.name)}</strong><p>${esc(row.condition||"조건 정보 없음")}</p>${mastery}</div><button class="admin-title-icon-edit" type="button" data-title-icon-edit="${esc(row.id)}">이미지 편집</button></article>`;}).join("")||`<div class="admin-empty-admin"><strong>조건에 맞는 칭호가 없습니다.</strong></div>`;target.querySelectorAll("[data-title-icon-edit]").forEach(button=>button.addEventListener("click",()=>openTitleIconEditor(button.dataset.titleIconEdit)));}
