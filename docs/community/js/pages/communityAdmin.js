...
import { renderMileage } from "./mileage.js?v=20260911authsingleton1";
...
function renderTitleCatalog(){const search=String(document.getElementById("titleSearch")?.value||"").toLowerCase(),category=document.getElementById("titleCategory")?.value||"",rows=titleRows.filter(row=>(!category||row.category===category)&&(!search||`${row.name} ${row.condition} ${row.champion||""} ${row.defaultTitle||""} ${row.overrideTitle||""}`.toLowerCase().includes(search)));const count=document.getElementById("titleCount"),target=document.getElementById("titleCatalog");if(count)count.textContent=`${rows.length}개`;if(!target)return;target.innerHTML=rows.map(row=>{const mastery=row.category==="챔피언 숙련",kind={normal:"일반",first:"서버 최초",multi:"복합 서버 최초"}[row.masteryKind]||row.masteryKind||"일반",source={champion:"챔피언 아이콘",custom:"커스텀",emoji:"기본"}[row.customIcon?"custom":row.iconSource]||"기본";return `<article><header><div class="admin-title-icon">${titleIconMarkup(row)}</div><div class="admin-title-heading"><small>${esc(row.category||"기타")}${row.seriesGroup?` · ${esc(row.seriesGroup)}`:""}</small><strong>${esc(row.name)}</strong><span class="admin-title-source">${esc(source)}</span></div><button class="admin-title-icon-edit" type="button" data-title-icon-edit="${esc(row.id)}">이미지 편집</button></header><div class="admin-title-body"><div><small>조건:</small><span>${esc(row.condition||"조건 정보 없음")}</span></div>${mastery?`<div><small>챔피언:</small><span>${esc(row.champion||"모든 챔피언")} · ${Number(row.games||0)}판 · ${esc(kind)}</span></div><div><small>기본 칭호:</small><span>${esc(row.defaultTitle||row.name)}</span></div>${row.overrideTitle?`<div><small>특수 칭호:</small><span>${esc(row.overrideTitle)}</span></div>`:""}${row.claimKey?`<div><small>최초 획득:</small><span>${esc(row.season||"현재 시즌")} · ${row.claimed?esc(row.claimantName||row.claimantId||"획득 완료"):"미획득"}</span></div>`:""}`:""}</div></article>`;}).join("")||`<div class="admin-empty-admin"><strong>조건에 맞는 칭호가 없습니다.</strong></div>`;target.querySelectorAll("[data-title-icon-edit]").forEach(button=>button.addEventListener("click",()=>openTitleIconEditor(button.dataset.titleIconEdit)));}
function openTitleIconEditor(key){const row=titleRows.find(item=>item.id===key),dialog=document.getElementById("titleIconDialog");if(!row||!dialog)return;editingTitleKey=key;document.getElementById("titleIconName").textContent=row.name;document.getElementById("titleIconPreview").innerHTML=titleIconMarkup(row)||"아이콘 없음";document.getElementById("titleIconStatus").textContent="";dialog.querySelector("[data-title-icon-remove]").disabled=!row.customIcon;dialog.showModal();}
async function saveTitleIcon(event){event.preventDefault();const status=document.getElementById("titleIconStatus"),file=new FormData(event.currentTarget).get("icon");if(!file?.size){status.textContent="이미지를 선택해주세요.";return;}status.textContent="저장 중...";const body=new FormData();body.append("icon",file);try{await adminRequest(`/api/community/admin/guilds/${encodeURIComponent(selectedGuild)}/titles/${encodeURIComponent(editingTitleKey)}/icon`,{method:"POST",body});document.getElementById("titleIconDialog").close();await loadTitleCatalog();}catch(error){status.textContent=error.message;}}
async function removeTitleIcon(){const status=document.getElementById("titleIconStatus");status.textContent="제거 중...";try{await adminRequest(`/api/community/admin/guilds/${encodeURIComponent(selectedGuild)}/titles/${encodeURIComponent(editingTitleKey)}/icon`,{method:"DELETE"});document.getElementById("titleIconDialog").close();await loadTitleCatalog();}catch(error){status.textContent=error.message;}}
