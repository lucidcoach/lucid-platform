...
let titleRows=[],levelIconState=null,editingTitleKey="",titlePreviewUrl="";
const TITLE_ICON_TYPES=["image/png","image/jpeg","image/webp"],TITLE_ICON_MAX_BYTES=512*1024,TITLE_ICON_SIZE=256;
function titlePanel(){return shell(`${panelTitle("칭호 / 업적","봇이 실제 사용하는 칭호 정의와 달성 조건입니다.")}<section class="admin-work-panel admin-level-icon-panel"><div><small>내전 레벨 아이콘</small><h3>프로필 공용 아이콘</h3><p>내전 레벨 배지에만 사용 · PNG/JPG/WebP 정사각형 256 × 256 · 512KB 이하</p></div><form id="levelIconForm"><span id="levelIconPreview" class="admin-level-icon-preview">🎮</span><input name="icon" type="file" accept="image/png,image/jpeg,image/webp" required><button class="admin-primary" type="submit">아이콘 변경</button></form><small id="levelIconStatus">이미지를 올리기 전에는 🎮 이모지를 사용합니다.</small></section><section class="admin-work-panel"><div class="admin-title-tools"><input id="titleSearch" type="search" placeholder="칭호 또는 조건 검색"><select id="titleCategory"><option value="">전체 대분류</option></select><select id="titleSubcategory"><option value="">전체 세부분류</option></select><b id="titleCount">0개</b></div><div id="titleCatalog" class="admin-title-grid"><div class="admin-empty-admin"><strong>불러오는 중...</strong></div></div></section><dialog id="titleIconDialog" class="admin-title-icon-dialog"><form id="titleIconForm"><button class="admin-dialog-close" type="button" data-title-icon-close aria-label="닫기">×</button><small>칭호 전용 이미지</small><h3 id="titleIconName"></h3><span id="titleIconPreviewLabel">현재 이미지</span><div id="titleIconPreview" class="admin-title-icon-preview"></div><small id="titleIconFileInfo">PNG/JPG/WebP · 256 × 256 · 512KB 이하</small><input name="icon" type="file" accept="image/png,image/jpeg,image/webp"><div class="admin-title-icon-sample"><span id="titleIconSample"></span><strong id="titleIconSampleName"></strong></div><div><button class="admin-primary" type="submit">저장</button><button type="button" data-title-icon-remove>이미지 제거</button></div><small id="titleIconStatus"></small></form></dialog>`);}
...
function closeTitleIconEditor(){clearTitlePreviewUrl();document.getElementById("titleIconDialog")?.close();}
function titleIconFileInfo(file,width,height){return `${file.name} · ${width} × ${height} · ${Math.ceil(file.size/1024)}KB`;}
async function validateTitleIcon(file){if(!TITLE_ICON_TYPES.includes(file?.type))throw new Error("지원하지 않는 이미지 형식입니다. PNG, JPG 또는 WebP를 선택해주세요.");if(file.size>TITLE_ICON_MAX_BYTES)throw new Error("이미지는 512KB 이하여야 합니다.");const url=URL.createObjectURL(file);try{const size=await new Promise((resolve,reject)=>{const image=new Image();image.onload=()=>resolve([image.naturalWidth,image.naturalHeight]);image.onerror=()=>reject(new Error("이미지 파일을 읽을 수 없습니다."));image.src=url;});if(size[0]!==TITLE_ICON_SIZE||size[1]!==TITLE_ICON_SIZE)throw new Error("이미지는 256×256이어야 합니다.");return {url,width:size[0],height:size[1]};}catch(error){URL.revokeObjectURL(url);throw error;}}
...
function openTitleIconEditor(key){const row=titleRows.find(item=>item.id===key),dialog=document.getElementById("titleIconDialog");if(!row||!dialog)return;clearTitlePreviewUrl();editingTitleKey=key;dialog.querySelector("form")?.reset();document.getElementById("titleIconName").textContent=row.name;document.getElementById("titleIconStatus").textContent="";showTitleIconPreview(row,titleIconMarkup(row),row.customIcon?"현재 이미지":"기본 이미지");dialog.querySelector("[data-title-icon-remove]").disabled=!row.customIcon;dialog.showModal();}
async function previewTitleIcon(file){const row=titleRows.find(item=>item.id===editingTitleKey),status=document.getElementById("titleIconStatus");if(!row||!file?.size)return;clearTitlePreviewUrl();try{const preview=await validateTitleIcon(file);titlePreviewUrl=preview.url;showTitleIconPreview(row,`<img src="${esc(preview.url)}" alt="변경 미리보기">`,"변경 미리보기",titleIconFileInfo(file,preview.width,preview.height));status.textContent="저장 전 미리보기입니다.";}catch(error){status.textContent=error.message;showTitleIconPreview(row,titleIconMarkup(row),row.customIcon?"현재 이미지":"기본 이미지");}}
async function saveTitleIcon(event){event.preventDefault();const status=document.getElementById("titleIconStatus"),file=new FormData(event.currentTarget).get("icon");if(!file?.size){status.textContent="이미지를 선택해주세요.";return;}try{await validateTitleIcon(file).then(preview=>URL.revokeObjectURL(preview.url));status.textContent="저장 중...";const body=new FormData();body.append("icon",file);await adminRequest(`/api/community/admin/guilds/${encodeURIComponent(selectedGuild)}/titles/${encodeURIComponent(editingTitleKey)}/icon`,{method:"POST",body});closeTitleIconEditor();await loadTitleCatalog();}catch(error){status.textContent=error.message;}}
async function removeTitleIcon(){if(!confirm("칭호 전용 이미지를 제거할까요?"))return;const status=document.getElementById("titleIconStatus");status.textContent="제거 중...";try{await adminRequest(`/api/community/admin/guilds/${encodeURIComponent(selectedGuild)}/titles/${encodeURIComponent(editingTitleKey)}/icon`,{method:"DELETE"});closeTitleIconEditor();await loadTitleCatalog();}catch(error){status.textContent=error.message;}}
