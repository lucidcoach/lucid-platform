...
function settingsCard(settings){
  ...
  return `<section class="mileage-card mileage-admin"><h2>서버 적립 규칙</h2><p class="mileage-muted">초기 권장: 내전 완료 20P · 승리 5P · 음성 30분당 5P. 승리 보너스는 완료 보상의 20~30%가 안정적입니다.</p><form id="mileageSettingsForm" class="mileage-form">
    ...
    <label><span>공식전적 동반게임 보상</span><select name="public_shared_play_enabled"><option value="1" ${r.public_shared_play_enabled?"selected":""}>사용</option><option value="0" ${!r.public_shared_play_enabled?"selected":""}>중지</option></select></label>
    ...
    ${numberInput("public_shared_play_points","동반게임 경기당 포인트",r.public_shared_play_points)}${numberInput("public_shared_play_daily_match_cap","동반게임 일일 인정 경기 (0=무제한)",r.public_shared_play_daily_match_cap)}
    ${numberInput("public_shared_play_min_duration_seconds","동반게임 최소 시간(초)",r.public_shared_play_min_duration_seconds)}
    <fieldset class="wide"><legend>동반게임 보상 대상</legend>${[["normal","일반"],["solo","솔랭"],["flex","자랭"],["aram","칼바람"]].map(([value,label])=>`<label><input type="checkbox" name="public_shared_play_categories" value="${value}" ${(r.public_shared_play_categories||[]).includes(value)?"checked":""}> ${label}</label>`).join("")}</fieldset>
    ...
    ${numberInput("invite_reward","초대 보상",r.invite_reward)}${numberInput("invite_min_account_days","초대 계정 최소 생성일",r.invite_min_account_days)}
    ${numberInput("invite_min_membership_days","초대 후 최소 활동 대기일",r.invite_min_membership_days)}
