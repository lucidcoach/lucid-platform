...
export function playerMatchCard(match, userId) {
  ...
  return `<article class="personal-match ${won ? "win" : "loss"}${special ? " special-match" : ""}">
    <div class="personal-summary">
      ...
      <div class="focus-champion">${champion ? `<img src="${escapeHtml(champion)}" alt="" loading="lazy">` : ""}${(player.spells?.length || player.perks?.length) ? `<div class="focus-loadout">${renderRuneSpells(player)}</div>` : ""}</div>
      <div class="focus-kda"><div class="match-tier-line"><span class="match-tier-badge">${escapeHtml(player.tier || "-")}</span><span>${escapeHtml(player.role || "")}</span></div><strong>${focusKda(player)}</strong><span>${player.deaths === 0 ? "Perfect" : `${Number(player.kda || 0).toFixed(2)} KDA`}</span></div>
