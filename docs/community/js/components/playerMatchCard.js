...
import { escapeHtml, focusKda, kdaClass, normalizeMode, relativeTime, scoreClass, tierClass } from "../utils.js?v=20260904f";
import { renderItems, renderProfileRuneSpells, renderBuildSummary } from "./loadout.js?v=20260904f";
  ...
  return `<article class="personal-match ${won ? "win" : "loss"}${special ? " special-match" : ""}">
    <div class="personal-summary">
      ...
      <div class="focus-visual">
        <div class="focus-topline"><div class="focus-champion">${champion ? `<img src="${escapeHtml(champion)}" alt="" loading="lazy">` : ""}${Number(player.level || 0) > 0 ? `<span class="champion-level">${Number(player.level)}</span>` : ""}</div>${runeSpells ? `<div class="focus-loadout-side">${runeSpells}</div>` : ""}</div>
        <div class="focus-bottomline"><div class="personal-items compact-items">${(player.items || []).length ? renderItems(player,6) : ""}</div>${achievements ? `<div class="match-achievements">${achievements}</div>` : ""}</div>
      </div>
