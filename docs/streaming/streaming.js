...
function renderAuction(){
  const a=auctionState,s=a.settings,current=a.current,player=a.players.find((item)=>item.id===current?.playerId),leader=a.teams.find((item)=>item.id===current?.teamId),myTeam=a.teams.find((item)=>item.id===a.myTeamId),nextBid=current?.teamId?current.amount+s.increment:s.minBid,seconds=current?Math.max(0,s.bidSeconds-Math.floor(Date.now()/1000-current.startedAt)):0;
  $("#auctionBroadcast").textContent=state?.workspace?.name||"";
  const mode=$("#auctionMode");mode.hidden=false;mode.textContent=a.canManage?(auctionPanel==="setup"?"관리자 설정":"진행 화면"):myTeam?`${myTeam.name} 팀장`:"팀장 입장 필요";
  if(!a.active){$("#auctionRoot").innerHTML=a.canManage?'<section class="auction-create"><h2>새 경매</h2><button data-auction-action="create">경매 만들기</button></section>':'<p class="empty">진행 중인 경매가 없습니다.</p>';return;}
  if(!a.canManage&&!a.myTeamId){$("#auctionJoinForm").hidden=false;$("#auctionRoot").innerHTML='<p class="empty">팀장 코드나 전용 링크로 입장해주세요.</p>';return;}
  const tabs=a.canManage?`<nav class="auction-view-tabs"><button class="${auctionPanel==="setup"?"active":""}" data-auction-panel="setup">경매 설정</button><button class="${auctionPanel==="stage"?"active":""}" data-auction-panel="stage">진행 화면</button></nav>`:"";
  if(a.canManage&&auctionPanel==="setup"){$("#auctionRoot").innerHTML=`${tabs}${auctionSetup(a,s)}`;return;}
  $("#auctionRoot").innerHTML=`${tabs}${auctionStage(a,s,current,player,leader,nextBid,seconds)}`;
}

function auctionSetup(a,s){
  const teamRows=a.teams.map((team)=>`<article class="auction-access-row"><div><strong>${escapeHtml(team.name)}</strong><small>${escapeHtml(team.captain)}</small></div><code>${escapeHtml(a.teamCodes?.[team.id]||"-")}</code><button class="quiet" data-copy-code="${team.id}">코드 복사</button><button class="quiet" data-copy-team="${team.id}">링크 복사</button>${!a.players.some((item)=>item.teamId===team.id)?`<button class="quiet danger" data-auction-action="removeTeam" data-id="${team.id}">삭제</button>`:""}</article>`).join("");
  const waiting=a.players.filter((item)=>!['sold','current'].includes(item.status)).map((item)=>`<div class="auction-player"><img src="${escapeHtml(imageUrl(item.imageUrl))}" alt="${escapeHtml(item.name)}"><div><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml([item.riotId,item.role,item.tier,item.note].filter(Boolean).join(' · '))}</small></div><span class="status-pill">${item.status==='unsold'?'유찰':'대기'}</span><button class="quiet" data-auction-action="removePlayer" data-id="${item.id}">삭제</button></div>`).join("");
  return `<section class="panel auction-settings"><div class="section-head"><h2>경매 설정</h2><button class="quiet danger" data-auction-action="finish">경매 종료</button></div><form data-auction-form="settings"><label>팀별 포인트<input name="points" type="number" min="1" value="${s.points}" required></label><label>최소 입찰가<input name="minBid" type="number" min="1" value="${s.minBid}" required></label><label>호가 단위<input name="increment" type="number" min="1" value="${s.increment}" required></label><label>입찰 시간<input name="bidSeconds" type="number" min="1" value="${s.bidSeconds}" required></label><button>설정 저장</button></form><div class="auction-add"><form data-auction-form="addTeam"><strong>팀 등록</strong><input name="name" placeholder="팀 이름" required><input name="captain" placeholder="팀장" required><button>팀 추가</button></form><form data-auction-form="addPlayer"><strong>참가자 등록</strong><label class="auction-file"><img data-upload-preview src="../logo.png" alt=""><span><b data-file-name>프로필 사진 선택</b><small>PNG · JPG · WebP, 512KB 이하</small></span><input name="image" type="file" accept="image/png,image/jpeg,image/webp"></label><input name="name" placeholder="참가자 이름" required><input name="riotId" placeholder="Riot ID"><select name="role"><option value="">라인</option>${options(roles)}</select><input name="tier" placeholder="티어"><input name="note" placeholder="참가자 정보"><button>참가자 추가</button></form></div></section><section class="panel"><div class="section-head"><h2>팀장 입장</h2><span class="muted">${a.teams.length}팀</span></div><div class="auction-access-list">${teamRows||'<p class="empty">등록된 팀이 없습니다.</p>'}</div></section><section class="panel"><div class="section-head"><h2>참가자</h2><span class="muted">${a.players.length}명</span></div><div class="auction-players">${waiting||'<p class="empty">등록된 참가자가 없습니다.</p>'}</div></section>`;
}

function auctionStage(a,s,current,player,leader,nextBid,seconds){
  const teams=a.teams.map((team)=>{const members=a.players.filter((item)=>item.teamId===team.id),canBid=Boolean(current&&(a.canManage||a.myTeamId===team.id));return `<article class="auction-team${a.myTeamId===team.id?' is-mine':''}"><header><div><strong>${escapeHtml(team.name)}</strong><small>${escapeHtml(team.captain)}</small></div><b>${team.points}P</b></header><ul>${members.map((item)=>`<li><span>${escapeHtml(item.role)} · ${escapeHtml(item.name)}</span><b>${item.price}P</b></li>`).join("")||'<li class="muted">선수 없음</li>'}</ul><div class="auction-team-actions">${canBid?`<button data-auction-action="bid" data-team-id="${team.id}">${nextBid}P 입찰</button>`:""}</div></article>`}).join("");
  const waiting=a.players.filter((item)=>!['sold','current'].includes(item.status)).map((item)=>`<div class="auction-player"><img src="${escapeHtml(imageUrl(item.imageUrl))}" alt="${escapeHtml(item.name)}"><div><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml([item.role,item.tier].filter(Boolean).join(' · '))}</small></div><span class="status-pill">${item.status==='unsold'?'유찰':'대기'}</span></div>`).join("");
  return `<section class="auction-stage panel">${player?`<img src="${escapeHtml(imageUrl(player.imageUrl))}" alt="${escapeHtml(player.name)}"><div class="auction-current"><span>${escapeHtml(player.role)} · ${escapeHtml(player.tier)}</span><h2>${escapeHtml(player.name)}</h2><p>${escapeHtml(player.note)}</p><strong>${current.amount||s.minBid}P ${leader?`· ${escapeHtml(leader.name)}`:""}</strong><b class="auction-timer">${seconds}초</b></div>`:'<p class="empty">진행 중인 선수가 없습니다.</p>'}${a.canManage?`<div class="auction-controls"><button data-auction-action="next" ${current?'disabled':''}>다음 선수</button><button data-auction-action="sold" ${!current?.teamId?'disabled':''}>낙찰</button><button class="quiet" data-auction-action="unsold" ${!current?'disabled':''}>유찰</button></div>`:""}</section><section class="auction-team-section"><div class="section-head"><h2>팀</h2><span class="muted">${a.teams.length}팀</span></div><div class="auction-teams">${teams||'<p class="empty">등록된 팀이 없습니다.</p>'}</div></section><section class="panel"><div class="section-head"><h2>대기 선수</h2><span class="muted">${a.players.filter((item)=>!['sold','current'].includes(item.status)).length}명</span></div><div class="auction-players">${waiting||'<p class="empty">대기 선수가 없습니다.</p>'}</div></section>`;
}
...
$("#auctionRoot").addEventListener("click",async(event)=>{const panel=event.target.closest("[data-auction-panel]");if(panel){auctionPanel=panel.dataset.auctionPanel;renderAuction();return;}const action=event.target.closest("[data-auction-action]");if(action){if(action.dataset.auctionAction==="finish"&&!confirm("현재 경매를 종료할까요? 기록은 다음 경매를 만들기 전까지 유지됩니다."))return;await auctionAction(action.dataset.auctionAction,{id:action.dataset.id||"",teamId:action.dataset.teamId||""});return;}const copyCode=event.target.closest("[data-copy-code]");if(copyCode){await navigator.clipboard.writeText(auctionState.teamCodes?.[copyCode.dataset.copyCode]||"");notice("입장 코드를 복사했습니다.");return;}const copy=event.target.closest("[data-copy-team]");if(copy){const token=auctionState.teamTokens?.[copy.dataset.copyTeam],url=`${location.origin}${location.pathname}?channel=${encodeURIComponent(currentSlug)}&view=auction&team=${encodeURIComponent(token)}`;await navigator.clipboard.writeText(url);notice("팀장 링크를 복사했습니다.");}});
$("#auctionJoinForm").addEventListener("submit",async(event)=>{event.preventDefault();const code=$("#auctionJoinCode").value.trim().toUpperCase();try{const {auction}=await request(`/api/streaming/auctions/join/${encodeURIComponent(code)}`);location.href=`${location.pathname}?channel=${encodeURIComponent(auction.slug)}&view=auction&team=${encodeURIComponent(auction.teamToken)}`;}catch(error){notice(errorText(error));}});
