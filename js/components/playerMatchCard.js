...
import { escapeHtml, focusKda, kdaClass, normalizeMode, relativeTime, scoreClass, tierClass } from "../utils.js?v=20260904b";
import { renderItems, renderRuneSpells, renderBuildSummary } from "./loadout.js?v=20260904b";
...
export function playerMatchCard(match, userId) {
  ...
  return `<article class="personal-match ${won ? "win" : "loss"}${special ? " special-match" : ""}">
    <div class="personal-summary">
      ...
      <div class="focus-champion">${champion ? `<img src="${escapeHtml(champion)}" alt="" loading="lazy">` : ""}${Number(player.level || 0) > 0 ? `<span class="champion-level">${Number(player.level)}</span>` : ""}${runeSpells ? `<div class="focus-loadout">${runeSpells}</div>` : ""}</div>
